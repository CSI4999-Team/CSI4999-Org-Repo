...
function App() {
  ...
  const handleBack = () => {
    ...
    if (currentStep === 2 || currentStep === 3) {
      ...
      // If they are viewing the results, let them go back to the upload form.
      setCurrentStep(3);
    }
  ...
  const handleSkip = () => {
    const userConfirmed = window.confirm(
      "Are you sure you want to skip and receive general feedback?"
    );
    if (userConfirmed) {
      ...
      <div className="choices-container">
        ...
        <div className="skip-choice">
          <button
            ...
            onClick={() => {
              setInputMethod("general");
              setCurrentStep(2);
            }}
          >
            Skip / General Feedback
          </button>
        </div>
      </div>
    );
